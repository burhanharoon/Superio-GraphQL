...
import { ALL_CANDIDATES } from '../services/graphQL/queries/candidate'
...
const Candidates = () => {
    ...
    return (
        <div className="border-b-2 p-12 grid grid-cols-4 lg:grid-cols-2 md:grid-cols-none md:grid-rows-4 gap-8">
            {data?.featuredCandidateMany.map(candidate =>
                <div key={candidate?._id} className="p-4 shadow flex flex-col justify-center gap-4 items-center border border-gray-100">
